...
export class ListadoComponent implements OnInit {
  ...
  listGlobal: any = [];
  listFlt: any = [];
  columns: string[] = ['Nombre', 'Apellido', 'Puesto', 'acciones'];
  ...
  private fetchItems():void{
    ...
    this.fetchSrv.request("POST","empleado/ObtenerPaginado",flt)
    .then(r =>{
      if( r.meta.status == "OK" ){
  ...
  irDetalle(valor) {
    console.log(valor);
    this.router.navigateByUrl('/empleado/registro',{state: valor});
  }

  eliminar(valor) {
    console.log(valor);
    this.utilSrv.presentAlert("Elimiinar Empleado","¿Desea eliminar este registro?")
    .then( r =>{
      if( r == "confirm"){
        this.loadginSrv.present()
        .then(()=>{
          this.fetchSrv.request("DELETE","empleado/Eliminar",valor)
          .then(r =>{
            if(r.meta.status == "OK"){
              this.messageSrv.success("Empleado eliminado exitosamente");
              this.fetchItems();
            }else{
  ...
  crear():void{
    this.router.navigateByUrl('/empleado/registro');
  }
}
